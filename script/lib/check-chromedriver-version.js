...
module.exports = function() {
  ...
  if (!chromedriverVer.startsWith('~')) {
    throw new Error(
      `electron-chromedriver version in script/package.json should start with a tilde to match latest patch version.`
    );
  }

  if (!mksnapshotVer.startsWith('~')) {
    throw new Error(
      `electron-mksnapshot version in script/package.json should start with a tilde to match latest patch version.`
    );
  }
